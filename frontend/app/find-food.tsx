...
export default function FindFood() {
  ...
  const [filter, setFilter] = useState<'all' | 'pickup' | 'byo'>('all');
  ...
  const reserve = (r: any, byo: boolean) => {
    Alert.alert(
      t('pickupReserved'),
      `${lang === 'ar' ? r.name_ar : r.name_en}\n${byo ? t('byoContainer') : t('selfPickup')}`,
      [{ text: 'OK' }]
    );
  };
  ...
  return (
    <Screen testID="find-food-screen" scroll={false} padded={false}>
      ...
      <Text style={[styles.sectionTitle, isRTL && styles.rtl, { paddingHorizontal: SPACING.lg }]}>
        {t('nearbyRestaurants')}
      </Text>
            ...
            >
              <View style={[styles.cardHead, isRTL && { flexDirection: 'row-reverse' }]}>
                ...
                <View style={{ flex: 1, marginHorizontal: SPACING.md }}>
                  <Text style={[styles.title, isRTL && styles.rtl]} numberOfLines={1}>{lang === 'ar' ? r.name_ar : r.name_en}</Text>
                  <Text style={[styles.sub, isRTL && styles.rtl]} numberOfLines={1}>{lang === 'ar' ? r.cuisine_ar : r.cuisine_en}</Text>
                </View>
...
const styles = StyleSheet.create({
  ...
  mapBg: {
    ...
    backgroundColor: '#E5EFE6', position: 'relative',
  ...
  water: {
    position: 'absolute', right: -20, top: -20, width: 120, height: 100,
    borderRadius: 60, backgroundColor: '#B8DCE5', opacity: 0.5,
  },
  ...
  pin: {
    ...
    marginLeft: -14, marginTop: -28, borderWidth: 2, borderColor: '#fff',
  },
  ...
  btn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, paddingVertical: 10, borderRadius: RADIUS.full },
  btnTxt: { color: '#fff', fontWeight: FONT.weight.bold, fontSize: FONT.size.xs, marginStart: 6 },
});
